...
namespace SteamSharp.Authenticators {
	...
	public class UserAuthenticator : ISteamAuthenticator {
		...
		/// <returns><see cref="UserAuthenticator"/> object for authentication of a <see cref="SteamClient"/> instance.</returns>
		public static UserAuthenticator ForProtectedResource( Cookie authCookie ) {
			return new UserAuthenticator {
		...
		public static SteamAccessRequestResult GetAccessCookieForUser( string username, string password, SteamGuardAnswer steamGuardAnswer = null, CaptchaAnswer captchaAnswer = null ) {
			...
			if( response.Cookies["steamLogin"] != null ) {
				...
				user.LoginKey = response.Cookies["steamLogin"].Value;
				//user.LoginKey = response.Cookies["steamLogin"].Value.Replace( "%7C", "|" ).Replace( "%7c", "|" ).Split( '|' )[1];
				//user.LoginKey = "93599CAD9CEF8B71DF1B4F050576C45A0F7A104E";
			}
		...
		public static string GetAccessTokenForUser( SteamUser user ) {
			...
			byte[] loginKey = Encoding.UTF8.GetBytes( user.LoginKey );
